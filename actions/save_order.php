...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $variationId = isset($_POST['variation_id']) ? (int)$_POST['variation_id'] : null; // Added variation ID handling
    ...
    try {
        if (isset($_POST['id']) && !empty($_POST['id'])) {
            ...
            $stmt = $pdo->prepare("UPDATE orders SET 
                ...
                delivery_date = ?, 
                model_id = ?, 
                variation_id = ?, // Added variation_id to update
                metal_type = ?,
                status = ?,
                notes = ?, 
                image_urls = ?
                WHERE id = ? AND company_id = ?");
            $stmt->execute([$clientName, $deliveryDateTime, $modelId, $variationId, $metalType, $status, $notes, $imageUrlsJson, $id, $companyId]);
            ...
            $stmt = $pdo->prepare("INSERT INTO orders 
                (user_id, company_id, client_name, delivery_date, model_id, variation_id, metal_type, status, notes, image_urls, created_at) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
            $stmt->execute([$userId, $companyId, $clientName, $deliveryDateTime, $modelId, $variationId, $metalType, $status, $notes, $imageUrlsJson]);
